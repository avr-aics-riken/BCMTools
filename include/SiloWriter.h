...
public:
  ...
  ///  @param[in] name データ名
  ///
  ///  @note セル中心で定義されたScalar3D<float>またはScaalr3D<double>にのみ対応．
  ///
  template <typename T>
  ...
  ///  @param[in] name データ名
  ///
  ///  @note セル中心で定義されたVector3D<float>またはVector3D<double>にのみ対応．
  ///
  template <typename T>
...
//  DBoptlist* optList = DBMakeOptlist(2);
 ...
 // double z[size0.z+1];
  ...
  ///  @param[in] name メッシュ名
  ///
  ///  @todo DBOPT_MB_FILE_NSとDBOPT_MB_BLOCK_NSを用いたファイル名とブロック名の記述．
  ///        (試したが，うまくいかない)
  ///
  void writeMultiMesh(const std::string& name) {
    ...
    const char* fileNS = "@test_%-d.silo@n";
    DBAddOption(optList, DBOPT_MB_FILE_NS, (void*)fileNS);
    const char* blockNS = "@block%-d/mesh@n";
    DBAddOption(optList, DBOPT_MB_BLOCK_NS, (void*)blockNS);
#endif
...
//  DBFreeOptlist(optList);
  ...
  void setGhostZones(const BlockBase* block, int gz_m[], int gz_p[]) {
    if (withGhostZones) {
      ...
      gz_m[0] = neighborInfo[X_M].isOuterBoundary() ? 0 : 1;
      gz_p[0] = neighborInfo[X_P].isOuterBoundary() ? 0 : 1;
      gz_m[1] = neighborInfo[Y_M].isOuterBoundary() ? 0 : 1;
      gz_p[1] = neighborInfo[Y_P].isOuterBoundary() ? 0 : 1;
      gz_m[2] = neighborInfo[Z_M].isOuterBoundary() ? 0 : 1;
      gz_p[2] = neighborInfo[Z_P].isOuterBoundary() ? 0 : 1;
    } else {
